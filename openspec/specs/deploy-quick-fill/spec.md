...
系统 SHALL 在部署操作页面展示当前所选分组的最近部署历史记录列表。

#### Scenario: 选择分组后显示历史记录
- **WHEN** 用户在部署页面选择了一个具体分组
- **AND** 该分组下的任务在 localStorage 中有历史记录
- **THEN** 在表单项上方显示「快捷填充」区域
- **AND** 按时间倒序展示该分组下所有任务的历史记录（最多 3 条）

#### Scenario: 未选分组时隐藏区域
- **WHEN** 用户未选择分组（显示「全部分组」）
- **THEN** 快捷填充区域不显示

#### Scenario: 切换分组后更新列表
- **WHEN** 用户切换到另一个分组
- **THEN** 快捷填充区域的内容立即更新为新分组下的历史记录
